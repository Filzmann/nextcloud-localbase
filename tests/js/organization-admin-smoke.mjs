...
for (const contract of ['orgs-directory-status', 'orgs-directory-groups', 'Verzeichnis- und LDAP-Kompatibilität', 'orgs-calendar-context-form', 'orgs-calendar-country', 'orgs-calendar-subdivision', 'orgs-calendar-timezone', 'Kalenderregion und fachliche Zeitzone']) if (!template.includes(contract)) throw new Error(`Admin-Markup fehlt: ${contract}`);
