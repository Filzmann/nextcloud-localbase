    ...
    class OrganizationExporter {
        ...
        markup() {
            return `<div class="orgs-export" role="group" aria-label="Organigramm exportieren">
                ...
                <span class="orgs-export-help">Exportiert wird der aktuell sichtbare Stand. PDF öffnet den Druckdialog für „Als PDF speichern“.</span>
        ...
        async onClick(event) {
            ...
            try {
                ...
                if (button.dataset.action === 'export-pdf') this.exportPdf(layout);
                this.feedback(button.dataset.action === 'export-pdf' ? 'Die Druckansicht wurde geöffnet. Wähle dort „Als PDF speichern“.' : 'Das Organigramm wurde exportiert.');
            } catch (error) {
        ...
        printHtml(svg) {
            return `<!doctype html><html lang="de"><head><meta charset="utf-8"><title>AD-Organigramm</title><style>@page { size: landscape; margin: 10mm; } html,body { margin: 0; background: #fff; } svg { display: block; width: 100%; height: auto; }</style></head><body>${svg}</body></html>`;
        }

        exportPdf(layout) {
            const printWindow = window.open('', '_blank');
            if (!printWindow) throw new Error('Die Druckansicht wurde vom Browser blockiert.');
            printWindow.opener = null;
            printWindow.addEventListener('load', () => { printWindow.focus(); printWindow.print(); }, { once: true });
            printWindow.document.open();
            printWindow.document.write(this.printHtml(this.toSvg(layout)));
