(function() {
    ...
    class OrganizationEditor {
        constructor({ container, form, onSave, onZoomChange = () => {} }) {
            this.container = container; this.form = form; this.onSave = onSave; this.definition = null;
            this.positions = [];
            this.sortDrag = null;
            this.editingRoleKey = null;
            this.hierarchyBoard = new window.LocalBase.components.HierarchyBoard({ container, onEditRole: roleKey => this.openRoleEditor(roleKey), onZoomChange });
        ...
        set(definition, positions = [], zoom = 100) { this.definition = clone(definition); this.positions = clone(positions); this.editingRoleKey = null; this.hierarchyBoard.setZoom(zoom, false); this.render(); }
        ...
        render() {
            ...
            const general = `<div class="orgs-general"><label>Präfix der Assistenzteams <input data-organization-field="teamGroupPrefix" value="${esc(data.teamGroupPrefix)}" required></label><label>Anzeigename der Assistenzteams <input data-organization-field="teamLabelPrefix" value="${esc(data.teamLabelPrefix)}" required></label><label>Maximale Kürzellänge <input data-organization-field="teamCodeMaxLength" type="number" min="1" max="64" value="${esc(data.teamCodeMaxLength)}" required></label><label>Titel des Leitungsblocks <input data-organization-field="staffBlockLabel" value="${esc(data.staffBlockLabel)}" required></label></div>`;
            const hierarchy = `<div class="orgs-hierarchy"><p>Bearbeite Gruppeneinstellungen über den Edit-Stift einer Diagrammkarte. Ziehe eine unterstellte Rolle am Rollengriff auf die Karte ihrer Leitung. Der separate waagerechte Positionsgriff ordnet Karten derselben Ebene rein visuell von links nach rechts; dafür stehen zusätzlich Pfeiltasten bereit. Verbindungen zwischen Bereichsrollen gelten automatisch in jedem jeweils gleichen Bereich.</p><div class="orgs-hierarchy-toolbar" aria-label="Hierarchieverbindung per Tastatur anlegen"><label>Leitung <select data-hierarchy-manager>${this.roleOptions(roles)}</select></label><label>Unterstellte Rolle <select data-hierarchy-target>${this.roleOptions(roles)}</select></label><button type="button" data-action="add-edge">Zuordnen</button></div><p class="orgs-feedback" data-hierarchy-feedback role="status" aria-live="polite"></p>${this.exporter.markup()}<div class="orgs-diagram-workspace" data-diagram-workspace><div class="orgs-organigram-shell" data-hierarchy-board></div><aside class="orgs-role-editor" data-role-editor aria-labelledby="orgs-role-editor-heading" hidden></aside></div></div>`;
            const roleOrder = `<div class="orgs-compact-content"><p>Diese Reihenfolge steuert Kalender und Gruppenlisten. Sie bleibt von der Links-rechts-Anordnung im Organigramm getrennt.</p><ol class="orgs-compact-list orgs-role-order" data-sort-list="roles">${roles.map(([key, role]) => this.roleOrderItem(key, role)).join('')}</ol></div>`;
            const areaSettings = `<div class="orgs-compact-content"><p>Öffne nur den Bereich, den du bearbeiten möchtest.</p><div class="orgs-compact-list" data-sort-list="areas">${areas.map(([key, area]) => this.areaCard(key, area)).join('')}</div></div>`;
            const vacationViews = `<div class="orgs-compact-content"><p>Die Karten legen fest, welche Rollen und Bereiche gemeinsam erscheinen.</p><div class="orgs-compact-list" data-organization-teams>${(data.organizationTeams || []).map(team => this.teamCard(team)).join('')}</div><button type="button" data-action="add-team">Urlaubsansicht hinzufügen</button></div>`;
            this.container.innerHTML = `<p class="orgs-feedback" data-organization-sort-feedback role="status" aria-live="polite"></p><div class="orgs-dashboard-grid" data-dashboard-scope="organization">${this.dashboardWidget('general', 'Allgemein', general)}${this.dashboardWidget('hierarchy', 'Direkte Hierarchie', hierarchy)}${this.dashboardWidget('role-order', 'Fachliche Gruppenreihenfolge', roleOrder)}${this.dashboardWidget('areas', 'Bürobereiche', areaSettings)}${this.dashboardWidget('vacation-views', 'Teamansichten im Urlaubsplaner', vacationViews)}</div>`;
        ...
        dashboardWidget(id, title, content) {
            const headingId = `orgs-${id}-heading`;
            const contentId = `orgs-${id}-content`;
            return `<section class="orgs-dashboard-widget" data-dashboard-widget data-widget-id="${esc(id)}" aria-labelledby="${esc(headingId)}"><header class="orgs-dashboard-header"><h4 id="${esc(headingId)}" data-dashboard-title>${esc(title)}</h4><div class="orgs-dashboard-actions"><button type="button" data-dashboard-move="-1" aria-label="${esc(title)} eine Position zurück verschieben">↑</button><button type="button" data-dashboard-handle draggable="true" aria-label="${esc(title)} per Drag-and-drop verschieben">⠿</button><button type="button" data-dashboard-move="1" aria-label="${esc(title)} eine Position weiter verschieben">↓</button><button type="button" data-dashboard-toggle aria-expanded="true" aria-controls="${esc(contentId)}" aria-label="${esc(title)} ein- oder ausklappen"><span aria-hidden="true">▾</span></button></div></header><div id="${esc(contentId)}" data-dashboard-content>${content}</div></section>`;
        }
